    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      {groupedRegistrations.map((group) => {
        ...
        return (
          <div
            key={group.teamId}
            className="border-2 border-cyan-500/70 bg-gradient-to-r from-cyan-950/90 via-black/95 to-black/90 px-4 py-2.5 shadow-[0_0_25px_rgba(0,242,254,0.25)] flex items-center gap-3.5 relative"
          >
                  ...
                  <div key={cat} className="flex items-center gap-1.5 bg-black/90 px-2 py-0.5 border border-white/20">
                    ...
                    {canManageThisTeam && (
                      <button
                        ...
                        title={`Retirar ${cat}`}
                        className="ml-0.5 text-rose-400 hover:text-rose-300 font-bold hover:bg-rose-500/20 px-1 text-xs transition-colors"
                      >
                        ×
                      </button>
                    )}
      ...
      {isLeader && league.registrationOpen && league.status === 'open' && (
        <button
          ...
          className="bg-cyan-500 hover:bg-cyan-400 text-black font-black px-4 py-3 text-xs md:text-sm uppercase tracking-wider rounded-none transition-colors flex items-center gap-2 shadow-[0_0_20px_rgba(0,242,254,0.35)] shrink-0"
        >
