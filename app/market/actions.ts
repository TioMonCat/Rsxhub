...
import { notifyDriverHired, notifyTeamInvitation, createNotification } from '@/lib/notifications-data'
    ...
    if (db) {
      try {
        ...
        await notifyDriverHired({
          userId: hiredUserId,
          teamName: teamDoc.data()?.name || 'un equipo',
      ...
      try {
        ...
        if (listingData?.user_id) {
          await notifyTeamInvitation({
            ...
            teamName: teamDoc.data()?.name || 'un equipo',
            message: customMessage || 'Únete a nuestro equipo para los próximos campeonatos.',
          })
        }
        revalidatePath('/market')
        return
