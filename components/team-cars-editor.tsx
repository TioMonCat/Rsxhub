...
import { Plus, Trash, Users, ShieldAlert, Filter, Trophy, AlertTriangle, Upload, FileArchive, Save, Loader2 } from 'lucide-react'
import type { CarEntry, TeamMemberOption, TakenDorsal, LeagueOption } from './team-cars-editor/types'
import { getSkinFileName } from './team-cars-editor/types'

export type { CarEntry, TeamMemberOption, TakenDorsal, LeagueOption }
export { getSkinFileName }
...
export function TeamCarsEditor({
  ...
  const [cars, setCars] = useState<CarEntry[]>(() => {
    if (initialCars && initialCars.length > 0) {
      return initialCars.map((car) => {
        const byLeague: Record<string, string[]> = car.driverUserIdsByLeague || (car as any).driver_user_ids_by_league || {}
        ...
        if (Object.keys(byLeague).length === 0 && Array.isArray(car.driverUserIds)) {
          if (carLeagueId) {
            byLeague[carLeagueId] = [...car.driverUserIds]
          }
        }
        return {
  ...
  const dorsalValidation = useMemo(() => {
    ...
    for (const car of cars) {
      ...
      // Check format (digits only, max 3 digits: e.g. 0, 00, 000, 7, 07, 777)
      if (!/^[0-9]{1,3}$/.test(d)) {
        errors[car.id] = 'Car number must be 1 to 3 numeric digits (e.g. 0, 00, 000, 7, 07, 777).'
        continue
      }
      ...
      if (internalCollision) {
        errors[car.id] = `Car number #${d} is duplicated in another car of your team for the same category and league.`
        continue
      }
      ...
      if (takenByOther) {
        const teamLabel = takenByOther.teamName || 'Another team'
        errors[car.id] = `Car number #${d} is already registered by team "${teamLabel}" in this category and league.`
      }
    }
  ...
  const updateCarDriver = (id: string, leagueKey: string, driverIndex: number, userId: string) => {
    ...
    setCars((prev) => {
      ...
      return prev.map((car) => {
        ...
        // If cleanUserId is assigned elsewhere in the same league, clean it out
        if (cleanUserId) {
          const carLeague = car.leagueId || 'general'
          if (leaguesOverlap(carLeague, targetLeagueKey)) {
            let changed = false
            Object.keys(currentByLeague).forEach((lKey) => {
              if (leaguesOverlap(lKey, targetLeagueKey)) {
...
export function SaveTeamCarsButton() {
  const { pending } = useFormStatus()

  return (
    <div className="w-full space-y-2 pt-4 border-t border-shell-line/50">
      {pending && (
        <div className="w-full bg-slate-900 h-2 overflow-hidden rounded-none border border-cyan-500/40 relative">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button
          type="submit"
          disabled={pending}
          className={`px-6 py-2.5 text-xs font-black uppercase tracking-wider text-white rounded-none transition-all flex items-center gap-2 cursor-pointer ${
            pending
              ? 'bg-cyan-950 border border-cyan-500/50 text-cyan-300 opacity-80 cursor-not-allowed'
              : 'bg-shell-accent hover:bg-red-700 shadow-lg'
          }`}
        >
          {pending ? (
            <>
