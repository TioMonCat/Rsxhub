...
// formatFirestoreValue and parseClassTags are imported from lib/firestore-utils.ts
      ...
      if (db) {
        try {
          ...
          steamSnap.docs.forEach((doc: any) => {
            ...
            if (data.steam_id) {
              steamByUserId.set(uid, data.steam_id)
            }
          })
          ...
          membersSnap.docs.forEach((doc: any) => {
            const d = doc.data()
            if ((d.role === 'owner' || d.role === 'manager') && d.user_id) {
              ownerUserIds.add(d.user_id)
            }
          })
          ...
          const users: PlatformDriverUser[] = profilesSnap.docs.map((doc: any) => {
            ...
            return {
              userId,
              displayName: d.display_name || d.displayName || 'Driver',
              ...
              isTeamOwner,
              createdAt: formatFirestoreValue(d.created_at) || new Date().toISOString(),
            }
          })
      ...
      if (sessionCookie) {
        try {
          ...
          const userTeam = teams.find((t) =>
            Array.isArray(t.members) && t.members.some((m: any) => m.userId === userId)
          )
          ...
          drivers.push({
            ...
            teamLogo: userTeam?.logoUrl || null,
            isTeamOwner,
