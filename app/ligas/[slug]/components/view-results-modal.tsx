      ...
      <div className="shell-panel border border-shell-line bg-[#090d16] max-w-4xl w-full p-5 md:p-6 text-white rounded-none shadow-[0_0_60px_rgba(0,0,0,0.9)] relative flex flex-col my-auto">
        ...
        <div className="border-b border-shell-line pb-3 mb-4">
          ...
          <p className="text-xs text-slate-400 mt-1 flex items-center gap-1.5">
            ...
            Verified official round classification by race organization.
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-3">
            ...
            <p className="text-xs font-mono uppercase tracking-wider">Loading {sessionFilter} classification...</p>
          </div>
        ) : results.length === 0 ? (
          <div className="border border-white/10 bg-black/40 p-8 text-center rounded-none my-4">
            ...
            <p className="text-sm font-bold text-slate-300 uppercase tracking-wider">No official {sessionFilter} results posted</p>
            <p className="text-xs text-slate-500 mt-1">League stewards have not uploaded official results for this {sessionFilter} session yet.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center justify-between border-b border-white/10 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                        {sessionFilter === 'qualifying' ? 'Qualifying Leaderboard' : 'Race Leaderboard'} - {catTag}
                      </span>
                    </div>
                    <span className="text-[11px] font-mono font-bold text-slate-400">
                      {categoryRows.length} Drivers Enrolled
                    </span>
                  </div>

                  {categoryRows.length === 0 ? (
                    <p className="text-xs text-slate-500 italic py-2">No classified positions recorded for this category.</p>
                  ) : (
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className="border-b border-white/10 bg-white/[0.03] text-slate-400 font-mono text-[10px] uppercase tracking-wider">
                            ...
                            <th className="p-2 w-20 text-center">CAR #</th>
                            {sessionFilter === 'qualifying' ? (
                              <th className="p-2 w-28 text-right">BEST LAP</th>
                            ) : (
                              <>
                                <th className="p-2 w-28 text-right">TIME / GAP</th>
                                <th className="p-2 w-24 text-right">POINTS</th>
                              </>
                            )}
                          </tr>
                        </thead>
